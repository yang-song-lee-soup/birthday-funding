...
describe('상품 페이지', () => {
    it('검색어가 없으면 안내 문구를 보여준다', async () => {
        ...
        expect(host.textContent).toContain('검색어를 입력해 네이버 쇼핑 상품을 찾아보세요.');
    });
    ...
    it('검색 결과를 목록으로 보여준다', async () => {
        ...
        expect(host.querySelector('a[href="https://example.com/cake"]')).not.toBeNull();
    });
    ...
    it('서비스 오류 메시지를 표시한다', async () => {
        getAll.mockResolvedValue([null, { message: '네이버 쇼핑 조회에 실패했습니다.' }]);
        await renderPage({ query: '케이크' });
    ...
    it('선택한 카테고리 링크를 활성화한다', async () => {
        ...
        const active = [...host.querySelectorAll('a')].find((el) => el.textContent?.includes('전자기기'));
        expect(active?.className).toContain('bg-yellow-100');
        expect(active?.getAttribute('href')).toBe('/product?query=%EC%A0%84%EC%9E%90%EA%B8%B0%EA%B8%B0&category=%EC%A0%84%EC%9E%90%EA%B8%B0%EA%B8%B0');
    });
});
