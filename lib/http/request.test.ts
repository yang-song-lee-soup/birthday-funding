...
describe('HTTP 요청', () => {
    ...
    it('GET 요청에 쿼리를 붙이고 JSON을 반환한다', async () => {
        ...
        const client = HttpFetch({ service: HttpService.BASE });
        ...
        expect(url.href).toBe('http://localhost:3000/api/product?query=%EA%B0%80%EB%B0%A9&display=12&sort=sim');
    ...
    it('POST 본문을 JSON으로 직렬화한다', async () => {
        ...
        const client = HttpFetch({ service: HttpService.TEST });

        await client.post('/posts', { title: 'hello' }).request();

        const [url, init] = fetchMock.mock.calls[0] as [URL, RequestInit];
        expect(url.href).toBe('https://jsonplaceholder.typicode.com/posts');
        expect(init.method).toBe('POST');
        expect(init.body).toBe(JSON.stringify({ title: 'hello' }));
    });

    it.each(['put', 'patch', 'delete'] as const)('%s 메서드로 요청한다', async (method) => {
        ...
        expect((fetchMock.mock.calls[0][1] as RequestInit).method).toBe(method.toUpperCase());
    });
    ...
    it('실패 응답은 AppError로 던진다', async () => {
        ...
        await expect(client.get('/missing').request()).rejects.toSatisfy((error: unknown) => {
